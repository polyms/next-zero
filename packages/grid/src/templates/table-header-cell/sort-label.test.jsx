...
describe('SortLabel', () => {
  it('should spread rest props to the root element', () => {
    const tree = shallow(<SortLabel {...defaultProps} data={{ a: 1 }} />);

    expect(tree.props().data).toEqual({ a: 1 });
  });

  it('should add custom class', () => {
    const tree = shallow(<SortLabel {...defaultProps} className="custom" />);

    expect(tree.find('span.custom').exists()).toBeTruthy();
  });

  it('should not add the onClick and onKeyDown handlers if disabled is true', () => {
    const { onClick, onKeyDown } = shallow(<SortLabel {...defaultProps} disabled />).props();
...
describe('Keyboard navigation', () => {
  ...
  it('can get focus', () => {
    const tree = shallow(<SortLabel {...defaultProps} />);

    expect(tree.prop('tabIndex')).toBe(0);
  });

  it('can not get focus if disabled is true', () => {
    const tree = shallow(<SortLabel {...defaultProps} disabled />);
  ...
  it('should handle the "Enter" and "Space" keys down', () => {
    ...
    const tree = shallow(<SortLabel {...defaultProps} onSort={onSort} />);
  ...
  it('should keep other sorting parameters on sorting change when the "Shift" key is pressed', () => {
    ...
    expect(onSort).toHaveBeenCalledWith({ keepOther: true, direction: undefined });
  });

...
describe('Pagination link', () => {
  ...
  it('should pass rest props to the root element', () => {
    const tree = shallow(<PaginationLink data={{ a: 1 }} />);
  ...
  it('should pass children to the root element', () => {
    ...
    expect(tree.find('.custom').exists()).toBeTruthy();
  });

  it('should not have aria-label by default', () => {
    const tree = shallow(<PaginationLink />);

    expect(tree.prop('aria-label')).toBe('');
  });
  ...
  describe('"next" prop', () => {
    it('should set aria-label', () => {
      ...
      expect(tree.prop('aria-label')).toBe('Next');
    });
    ...
    it('should render children if passed', () => {
      ...
      expect(tree.find('a > span > .custom').exists()).toBeTruthy();
    });

    it('should render angle quote mark if no children passed', () => {
      const tree = shallow(<PaginationLink next />);

      expect(tree.findWhere(n => n.key() === 'caret').text()).toBe('»');
    });
    ...
    it('should render aria label element for screen readers', () => {
      ...
      expect(srSpan.childAt(0).text()).toBe('Next');
    });
  });
  ...
  describe('"previous" prop', () => {
    it('should set aria-label', () => {
      ...
      expect(tree.prop('aria-label')).toBe('Previous');
    });
    ...
    it('should render aria label element for screen readers', () => {
      ...
      expect(srSpan.key()).toBe('sr');
      expect(srSpan.childAt(0).text()).toBe('Previous');
    });
  });
});

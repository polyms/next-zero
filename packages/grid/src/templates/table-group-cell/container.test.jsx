...
describe('TableGroupCell', () => {
  describe('Container', () => {
    it('should have correct classes', () => {
      ...
      expect(tree.is('.position-sticky.dx-g-nz-fixed-group-cell'));
    });

    it('should apply style', () => {
      const tree = shallow(<Container position="13px" />);

      expect(tree.prop('style')).toEqual({ left: '13px' });
    });
    ...
    it('should render children', () => {
      ...
      expect(tree.find('.child')).toBeTruthy();
    });

    it('should pass custom class to the root element', () => {
      const tree = shallow(<Container className="custom-class" />);

      expect(tree.is('.position-sticky.dx-g-nz-fixed-group-cell.custom-class')).toBeTruthy();
    });

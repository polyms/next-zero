...
describe('TableGroupCell', () => {
  describe('IndentCell', () => {
    ...
    it('should merge custom style', () => {
      const tree = shallow(<IndentCell style={{ background: 'yellow' }} position={13} />);
    ...
    it('should pass custom class to the root element', () => {
      const tree = shallow(<IndentCell className="custom-class" />);

      expect(tree.is('.position-sticky.dx-g-nz-fixed-cell.custom-class')).toBeTruthy();
    });

    it('should pass rest props to root component', () => {
      const tree = shallow(<IndentCell data={{ a: 1 }} />);

      expect(tree.prop('data')).toEqual({
        a: 1,
      });
    });
  });

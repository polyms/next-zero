...
import { Pagination as PaginationBS4, PaginationItem, PaginationLink } from '../parts/pagination';
...
describe('Pagination', () => {
  it('can show info about rendered pages', () => {
    ...
    const tree = shallow(<Pagination {...defaultProps} getMessage={getMessage} />);

    expect(getMessage).toHaveBeenCalledWith('info', { from: 11, to: 20, count: 96 });
    expect(tree.find('span > span').text()).toBe('info');
  });

  it('can render pagination arrows', () => {
    const onCurrentPageChange = jest.fn();
    const paginations = shallow(<Pagination {...defaultProps} onCurrentPageChange={onCurrentPageChange} />).find(
      PaginationBS4
    );
  ...
  it('disables the prev arrow if the first page is active', () => {
    const paginations = shallow(<Pagination {...defaultProps} currentPage={0} />).find(PaginationBS4);
  ...
  it('disables the next arrow if current page equals to total page count', () => {
    const paginations = shallow(<Pagination {...defaultProps} currentPage={9} pageSize={5} />).find(PaginationBS4);
